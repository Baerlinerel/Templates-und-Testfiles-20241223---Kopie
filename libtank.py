...
class VierTank:
    ...
    def model(self,t,x,controller):
        ...
        Q01 = self.get_flow_from_input(1, u[0])  # Zufluss in Tank 1 durch Pumpe 1
        Q02 = self.get_flow_from_input(2, u[1])  # Zufluss in Tank 2 durch Pumpe 2

        # Abflüsse berechnen
        Q12 = self.get_outflow_from_level(1, 2, x[0])  # Tank 1 → Tank 2
        Q13 = self.get_outflow_from_level(1, 3, x[0])  # Tank 1 → Tank 3
        Q23 = self.get_outflow_from_level(2, 3, x[1])  # Tank 2 → Tank 3
        Q24 = self.get_outflow_from_level(2, 4, x[1])  # Tank 2 → Tank 4
        Q34 = self.get_outflow_from_level(3, 4, x[2])  # Tank 3 → Tank 4
        Q30 = self.get_outflow_from_level(3, 5, x[2])  # Tank 3 → Reservoir
        Q40 = self.get_outflow_from_level(4, 5, x[3])  # Tank 4 → Reservoir

        # Ableitungen der Zustandsgrößen berechnen
        dx[0] = (Q01 - Q12 - Q13) / self.st.AT
        dx[1] = (Q02 + Q12 - Q23 - Q24) / self.st.AT
    ...
    def output(self, t, x, controller = None):
        ...
        if np.isscalar(t):# Einzelausgabe (Skalar), die Funktion np.isscalar prüft ob der Wert Sakalar(Wert: 1,42,3.12, 1+j, True/False) ist. 
            y = np.zeros((2,)) #Erstellt ein 1D-Array mit 2 Elementen die zunächst alle mit 0 gefüllt sind 
            y[0] = x[2]  # y1 = x3
            y[1] = x[3]  # y2 = x4
            # hier sind nur x3 und x4 intressant, da Sie die Ausgänge des Systems sind und daher auch =yn sind. weil x[0] = Tank1 ist, Ist x[2]=y[0] 
        else: # Mehrfache Ausgabe (Vektor) 
            y = np.zeros((2, t.shape[0])) #erstellt ein 2D-Array um mehrere Zeitpunkte zu speichern 
            y[0, :] = x[2, :]  # y1 = x3
            y[1, :] = x[3, :]  # y2 = x4
        ######-------!!!!!!Aufgabe Ende!!!!!!-------########
        return y 
    ...
    def equilibrium(self,y):
        ...
        equi.status = 0
        equi.x = np.zeros((4,))  # Füllstände der vier Tanks
        equi.u = np.zeros((2,))  # Spannungen der zwei Pumpen
        equi.y = np.zeros((2,))  # Ausgänge (hier: Füllstände in den unteren Tanks)

    # Berechnungen:
        try:
            # Setze den gewünschten Ausgang (Füllstände der unteren Tanks)
            equi.y[:] = y
            ...
            # Iterative Lösung zur Sicherstellung von dx1 = dx2 = dx3 = dx4 = 0
            for _ in range(10):  # Maximal 10 Iterationen zur Konvergenz
                # Berechnung der Abflüsse
                q34 = self.get_outflow_from_level(3, 4, x3)
                q40 = self.get_outflow_from_level(4, 5, x4)

                # Zuflüsse berechnen
                q1 = self.get_flow_from_input(1, equi.u[0])  # Zufluss für Tank 1
                q2 = self.get_flow_from_input(2, equi.u[1])  # Zufluss für Tank 2

                # Separierte Berechnung von q13 und q23
                q13 = self.get_outflow_from_level(1, 3, x3)
                q23 = self.get_outflow_from_level(2, 3, x3)

                q24 = self.get_outflow_from_level(2, 4, x4)
                q12 = self.get_outflow_from_level(1, 2, x3)

                # Berechnung der Füllstände der oberen Tanks
                x1 = (q1 - q12 - q13) / self.st.AT
                x2 = (q2 + q12 - q23 - q24) / self.st.AT

                # Abflüsse für die unteren Tanks berechnen
                q3 = self.get_outflow_from_level(3, 0, x3)
                q4 = self.get_outflow_from_level(4, 0, x4)

                # Aktualisiere x3 und x4 basierend auf den Abflüssen
                x3 = (q13 + q23 - q3 - q34) / self.st.AT
                x4 = (q24 + q34 - q4 - q40) / self.st.AT

            equi.x[:] = [x1, x2, x3, x4]

            # Berechnung der Pumpenspannungen
            u1 = self.get_input_from_flow(1, q1)
            u2 = self.get_input_from_flow(2, q2)

            equi.u[:] = [u1, u2]

            # Statusprüfung
            if any(equi.x < 0):
                equi.status |= 1  # negativer Füllstand
            if any(equi.x > 30):
                equi.status |= 2  # Füllstand überschritten
            if any(equi.u > 12):
                equi.status |= 4  # Spannung überschritten

        except Exception as e:
            warnings.warn(f"Fehler bei der Berechnung der Ruhelage: {e}")
            equi.status = -1
